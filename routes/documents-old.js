...
router.post('/upload', authenticateToken, authorizeRole(['manager']), upload.single('file'), async (req, res) => {
  try {
    ...
    let communityName;
    if (req.body.skip_extraction === 'true' && req.body.community_name) {
      // Use manually provided community name
      communityName = req.body.community_name.trim();
      console.log('🏘️  Using manual community name:', communityName);
    } else {
      // Automatically extract community name from document text
  ...
  try {
    ...
    for (const file of req.files) {
      try {
        ...
        let communityName;
        if (req.body.skip_extraction === 'true' && req.body.community_name) {
          // Use manually provided community name (applies to all files in batch)
          communityName = req.body.community_name.trim();
